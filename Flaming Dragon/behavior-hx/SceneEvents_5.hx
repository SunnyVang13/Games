	...
	override public function init()
	{
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				g.drawString("" + "Kill 7 Enemies", 25, 435);
				g.drawString("" + "Level 5", 530, 435);
				if((Engine.engine.getGameAttribute("Level Complete") && true))
				{
					g.drawString("" + "You Win!", 244, 210);
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((Engine.engine.getGameAttribute("Enemies Killed") >= 84))
				{
		...
		/* ======================== Actor of Type ========================= */
		addCollisionListener(getActor(14), function(event:Collision, list:Array<Dynamic>):Void
		{
