...
class ActorEvents_34 extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((_HealthPoints <= 0))
				{
					createRecycledActor(getActorType(30), 300, 62, Script.FRONT);
		...
		runPeriodically(1000 * 5, function(timeTask:TimedTask):Void
		{
			if(wrapper.enabled)
			{
				createRecycledActor(getActorType(60), actor.getX(), actor.getY(), Script.FRONT);
			}
		}, actor);
