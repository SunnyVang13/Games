...
class Design_3_3_HeroMovement extends ActorScript
{
	public var _Grounded:Bool;
	public var _WalkingAnimation:String;
	...
	public function new(dummy:Int, actor:Actor, dummy2:Engine)
	{
		...
		_Grounded = false;
		nameMap.set("Walking Animation", "_WalkingAnimation");
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((isKeyPressed("up") && _Grounded))
				{
					...
					actor.setAnimation("" + _JumpAnimation);
					actor.applyImpulse(0, -1, 50);
				}
				if(isKeyDown("left"))
				{
					actor.setXVelocity(-10);
					actor.setAnimation("" + _WalkingAnimation);
				}
